...
public class ElementsOfRealEstateTest {
    ...
    @Test(expected = ElementsOfRealEstate.AddingSameTile.class) public void
    addingSameTile_thenRuntimeException() {
        element = element
                .add(tile, direction)
                .add(tile, direction);
    }
    ...
    @Test(expected = ElementsOfRealEstate.TileDoesNotExist.class) public void
    getTileDirectionSet_ofNotAddedTile_thenRuntimeException() {
        element.add(tile, direction);
        element.getTileDirectionSet(tile2);
    }

    @Test public void
    testAddTileAndThreeTileDirections() {
    ...
    exceptionAddingSetWithDuplicate() {
        element = element.add(tile, direction);
    ...
    equalsTest () {
        element = element
                .add(tile, EAST, WEST)
                .add(tile2, CENTER);
        ElementsOfRealEstate sameElement = new ElementsOfRealEstate()
                .add(tile2, CENTER)
                .add(tile, EAST, WEST);
        ElementsOfRealEstate differentElement = new ElementsOfRealEstate()
                .add(tile, EAST)
                .add(tile2, CENTER);

        assertThat(element, is(not(differentElement)));
        assertThat(element, is(sameElement));
    }

    @Test public void
    addFromSupplierTest () {
        Map<Tile, Set<TileDirection>> data = new HashMap<>();
        data.put(tile, new HashSet<>(Collections.singletonList(direction)));
        data.put(tile2, new HashSet<>(Arrays.asList(EAST, WEST, SOUTH)));

        ElementsOfRealEstate correct = new ElementsOfRealEstate();
        correct = correct.add(tile, direction).add(tile2, EAST, WEST, SOUTH);
        element = element.addFromSupplier(() -> data);

        assertEquals(element, correct);
        assertThat(element, is(equalTo(correct)));
